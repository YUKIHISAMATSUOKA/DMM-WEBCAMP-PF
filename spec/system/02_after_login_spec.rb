...
describe '[STEP2] カスタマーログイン後のテスト' do
  let(:customer) { create(:customer) }
  let!(:other_customer) { create(:customer) }
  let!(:shop) { create(:shop, customer: customer) }
  let!(:other_shop) { create(:shop, customer: other_customer) }
  ...
  describe 'ショップ作成のテスト' do
    before do
      visit new_shop_path
    end

    context 'ショップ作成成功のテスト' do
      before do
        fill_in 'shop[name]', with: Faker::Lorem.characters(number: 10)
        fill_in 'shop[introduction]', with: Faker::Lorem.characters(number: 20)
        fill_in 'shop[post_code]', with: Faker::Number.number(digits: 7)
        fill_in 'shop[address]', with: Faker::Address.street_address
        fill_in 'shop[telephone_number]', with: Faker::Number.number(digits: 9)
      end
      it '自分のショップが正しく保存される' do
        change(customer.shop, :count).by(1)
      end
    end
  end
